import './styles.css';
import { DEMO_NOTES, clozePassage, deletionChoices, gradeNote, makeNote, type ReadingNote, type RecallGrade } from '../../shared/model';

const PRODUCT = 'Reading Margin Recall';
const SLUG = 'reading-margin-recall';
...
function landing(): string {
  ...
  <section class="paid section-pad" id="study-edition" aria-labelledby="paid-title"><div class="price-stamp"><strong>$12</strong><span>one time</span></div><div><p class="eyebrow">Optional Study Edition</p><h2 id="paid-title">Add focused review views</h2><p>Keep every core tool free. Study Edition adds difficult-only and source-specific review filters.</p><ul><li>One-time purchase</li><li>No subscription or account</li><li>Sociobot is the merchant of record</li></ul><div class="paid-actions"><a class="button primary" href="${BUY_URL}" rel="external">Buy Study Edition <span class="sr-only">(external checkout)</span></a><button class="button secondary" data-action="show-license">Restore a purchase</button></div><form id="license-form" class="license-form hidden"><label for="license">License token</label><div><input id="license" name="license" autocomplete="off" required><button class="button secondary" type="submit">Verify license</button></div><p id="license-status" aria-live="polite">Paste the token from your receipt.</p></form><p class="legal-line">Buying means you accept the <a class="nav-link" href="/terms">terms</a> and <a class="nav-link" href="/privacy">privacy notice</a>.</p></div></section>`);
}
...
function privacy(): string {
  return shell(`<article class="legal section-pad"><p class="eyebrow">Privacy · updated 28 August 2026</p><h1>Your notes stay in your browser</h1><h2>What is stored</h2><p>Passages, glosses, source links, review dates, settings, and an optional license token stay in browser storage.</p><h2>What leaves the device</h2><p>Nothing leaves during capture, review, export, or demo use. If you verify a purchase, the license token goes to the Sociobot billing API.</p><h2>Demo data</h2><p>The demo uses keys that start with <code>demo:</code>. Starting for real deletes those demo keys and never copies them.</p><h2>Extension access</h2><p>The extension reads only text you actively select. It does not fetch full pages, book files, or protected content.</p><h2>Delete your data</h2><p>Delete individual notes in My notes. Clearing this site’s browser storage removes everything.</p><h2>Contact</h2><p>Email <a href="mailto:privacy@sociobot.in">privacy@sociobot.in</a> with privacy questions.</p></article>`);
}

function terms(): string {
  return shell(`<article class="legal section-pad"><p class="eyebrow">Terms · updated 28 August 2026</p><h1>Use your own reading material</h1><h2>Your responsibility</h2><p>Capture only text you may access. Do not use the extension to copy whole works or bypass access controls.</p><h2>The free tools</h2><p>Capture, review, source links, JSON export, and JSON import are free.</p><h2>Study Edition</h2><p>Study Edition costs $12 once. It adds focused review filters on one browser profile at a time.</p><p>Sociobot and Dodo are the merchant of record. Refunds are handled through the receipt, and a refund revokes the license.</p><h2>No warranty</h2><p>The software is provided as is. Keep a JSON backup of notes you cannot replace.</p><h2>Contact</h2><p>Email <a href="mailto:support@sociobot.in">support@sociobot.in</a> for purchase help.</p></article>`);
}
...
function captureNote(event: SubmitEvent) {
  ...
  try { new URL(input.sourceUrl); } catch { error.textContent = 'The source link is not a valid URL. Paste the full address.'; return; }
  const notes = readNotes();
...
async function importNotes(event: Event) {
  ...
  try {
    ...
    saveNotes(parsed.notes); render(); announce(`${parsed.notes.length} notes imported.`);
...
async function restoreLicense(event: SubmitEvent) {
  event.preventDefault(); const form = event.currentTarget as HTMLFormElement; const token = String(new FormData(form).get('license') ?? '').trim();
  const status = document.querySelector<HTMLElement>('#license-status')!; status.textContent = 'Checking this license…';
  try {
    ...
    if (!result.valid) { status.textContent = 'This license is not active. Check the token or buy Study Edition.'; return; }
    localStorage.setItem(`sb_license:${SLUG}`, token); localStorage.setItem(`sb_license_verdict:${SLUG}`, JSON.stringify({ valid: true, checkedAt: Date.now() })); status.textContent = 'Study Edition is active on this browser.';
