...
check(missingHtml.includes('<meta name="robots" content="noindex"'), 'The product 404 is missing its noindex directive.');
const missingUrls = [...missingHtml.matchAll(/https?:\/\/[^"'\s<]+/g)].map((match) => new URL(match[0]));
check(missingUrls.every((url) => url.origin === base.origin), 'The product 404 contains a third-party URL.');
check(missingHtml.includes('aria-label="Main navigation"'), 'The product 404 is missing main navigation.');
check(missingHtml.includes('aria-label="Footer navigation"'), 'The product 404 is missing footer navigation.');
check(missingHtml.includes('<meta name="description"'), 'The product 404 is missing its description.');
check(missingHtml.includes('rel="apple-touch-icon"'), 'The product 404 is missing its apple-touch icon.');
...
try {
  ...
  check(new URL(desktopPage.url()).searchParams.get('demo') === '1', 'The live sample action did not enter ?demo=1.');
  check(await desktopPage.getByText('3 saved notes').isVisible(), 'The live demo did not load three sample notes.');
  check(await desktopPage.getByText('Demo — sample data, nothing is saved').isVisible(), 'The live demo banner is missing.');
  check(await desktopPage.getByRole('button', { name: 'Reveal sample answer' }).isVisible(), 'The live demo does not show an actionable sample review first.');
  ...
  check(await desktopPage.getByText('4 saved notes').isVisible(), 'The live demo capture did not save into the demo store.');
  ...
  check(await desktopPage.getByText('3 saved notes').isVisible(), 'The live demo did not reload offline with three notes.');
